...
Data description: where does it come from, what is it about what variables we chose: Price variable is the variable we want to predict How did we do it:
...
groups = c("price", "lot_size", "age", "land_value", "pct_college", "living_area", "bedrooms") summary = house %\>% select(price, lot_size, age, land_valie, pct_college, living_area, bedrooms) %\>% summarise_all(list(mean, sd, max, min)) summaries = c(rep(c("Mean", "SD","Max", "Min"), each=7)) columns= c(rep(groups, 4)) summary_data = as.numeric(summary\[1,\]) summary_frame \<- data.frame(summaries, columns, summary_data)

summary_wide = summary_frame %\>% tidyr::pivot_wider( id_cols = columns, names_from = summaries, values_from = summary_data ) summary_wide$Mean = round(summary_wide$Mean, 2)\
summary_wide$SD = round(summary_wide$SD, 2) summary_wide$Max = format(round(summary_wide$Max, 2), nsmall = 2) summary_wide$Min = format(round(summary_wide$Min, 2), nsmall = 2) knitr::kable(summary_wide)

## scatter plot of variables --\> Yi had a really nice one with all the variables
...
```{r}
# $$\hat{Price} = `r step.back.aic$coefficients[[1]]` + `r step.back.aic$coefficients[[2]]` \times Lot.Size + `r step.back.aic$coefficients[[3]]` \times withWaterfront + `r step.back.aic$coefficients[[4]]` \times Age + `r step.back.aic$coefficients[[5]]` \times Land.Value + `r step.back.aic$coefficients[[6]]` \times isNew.Construct + 
#     `r step.back.aic$coefficients[[7]]` \times withCentral.Air + `r step.back.aic$coefficients[[8]]` \times HotAirHeatType + `r step.back.aic$coefficients[[9]]` \times HotWaterHeatType + `r step.back.aic$coefficients[[10]]` \times NoHeat + `r step.back.aic$coefficients[[11]]` \times Living.Area + `r step.back.aic$coefficients[[12]]` \times Bedrooms + `r step.back.aic$coefficients[[13]]` \times Bathrooms + 
#     `r step.back.aic$coefficients[[14]]` \times Rooms$$
